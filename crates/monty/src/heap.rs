...
pub(crate) struct HeapReader<'a, H: ContainsHeap> {
    pub(crate) heap: &'a mut H,
    /// Makes the lifetime `'a` invariant.
    phantom: PhantomData<fn(&'a H) -> &'a H>,
}

impl<H: ContainsHeap> HeapReader<'_, H> {
    ...
    pub fn with<R>(heap: &mut H, f: impl for<'a> FnOnce(&'a mut HeapReader<'a, H>) -> R) -> R {
        f(&mut HeapReader {
            heap: &mut *heap,
            phantom: PhantomData,
        })
...
impl<'a, H: ContainsHeap> HeapReader<'a, H> {
...
impl<H: ContainsHeap> ContainsHeap for HeapReader<'_, H> {
    type ResourceTracker = H::ResourceTracker;
...
impl<T: ResourceTracker> Heap<T> {
        ...
        loop {
            ...
            if entry.refcount.get() > 1 {
                ...
                if let Some(mut data) = value.data {
                    ...
                    data.0.get_mut().py_dec_ref_ids(&mut work_stack);
                    drop(data);
                }
            }
    ...
    pub fn call_attr_raw(
        ...
        let result = data.0.get_mut().py_call_attr_raw(id, vm, attr, args);
...
mod heap_reader_tests {
    ...
    fn read_two_entries_then_deref_first() {
        ...
        HeapReader::with(&mut heap, |reader| {
            ...
            let HeapReadOutput::List(a) = reader.read(list_a) else {
                panic!("Expected List")
            };
            ...
            let HeapReadOutput::List(b) = reader.read(list_b) else {
                panic!("Expected List")
            };
    ...
    fn read_same_entry_twice_then_deref_both() {
        ...
        HeapReader::with(&mut heap, |reader| {
            let HeapReadOutput::List(a) = reader.read(list_id) else {
                panic!("Expected List")
            };
            let HeapReadOutput::List(b) = reader.read(list_id) else {
                panic!("Expected List")
            };
    ...
    /// Read two entries, use shared access to iterate both, then get_mut to replace.
    #[test]
