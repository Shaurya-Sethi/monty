...
impl HeapReader<'_> {
    ...
    pub fn with<R>(heap: &mut Heap, f: impl for<'a> FnOnce(&'a mut HeapReader<'a>) -> R) -> R {
        f(&mut HeapReader {
            heap: &mut *heap,
            phantom: PhantomData,
        })
...
///
/// Stores the resource tracker as `Box<dyn ResourceTracker>` for dynamic dispatch,
/// eliminating the generic parameter that previously propagated through 15+ types.
/// The performance cost is negligible: the hottest path (`check_time`) is a single
/// vtable call, dwarfed by the actual work done in any non-trivial tracker.
#[derive(Debug)]
...
impl<'de> serde::Deserialize<'de> for Heap {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        use crate::resource::TrackerState;
        ...
        Ok(Self {
            ...
            tracker: fields.tracker.into_tracker(),
...
impl Heap {
    ...
    pub fn new(capacity: usize, tracker: Box<dyn ResourceTracker>) -> Self {
        let this = Self {
    ...
    /// # Panics
    /// Panics if the value ID is invalid or the value has already been freed.
    pub fn get_or_compute_hash(vm: &mut VM<'_, '_>, id: HeapId) -> Result<Option<u64>, ResourceError> {
        let entry = vm
            .heap
            .entries
            .get_mut(id.index())
...
fn hash_element_at<'h>(
    read_element: impl for<'r> Fn(&'r HeapReader<'h>) -> &'r Value,
    vm: &mut VM<'h, '_>,
) -> Result<Option<u64>, ResourceError> {
