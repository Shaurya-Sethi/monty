...
///
/// Also carries the [`ResourceTracker`] borrow used by every resource-sensitive
/// operation (`allocate`, `dec_ref`, `track_growth`, …). The tracker rides on
/// the same brand lifetime `'a` so callers can project `&'a T` via
/// [`Self::tracker`] without producing a sub-borrow of `&mut Heap` — this is
/// what makes [`StringBuilder`](crate::string_builder::StringBuilder) able to
/// outlive heap mutation.
///
...
pub(crate) struct HeapReader<'a, T: ResourceTracker> {
    ...
    /// Borrowed resource tracker. Re-branded down to the brand lifetime `'a` so
    /// that [`Self::tracker`] can hand out `&'a T` without re-borrowing `&self`.
...
impl<T: ResourceTracker> HeapReader<'_, T> {
    ...
    ///
    /// The `tracker` borrow is re-branded to the same `'a` as the heap, so callers
    /// inside the closure can project it through [`Self::tracker`] for use by
    /// [`StringBuilder`](crate::string_builder::StringBuilder) and other tracker
    /// consumers that need to outlive heap mutation.
...
impl<'a, T: ResourceTracker> HeapReader<'a, T> {
    /// Returns the borrowed resource tracker with the outer brand lifetime.
    ///
    /// The returned `&'a T` is **not** a sub-borrow of `&self` — it projects the
    /// tracker field's brand lifetime directly. That is what lets
    /// [`StringBuilder`](crate::string_builder::StringBuilder) (or any other
    /// tracker consumer) outlive subsequent `&mut self.heap` mutations.
    #[inline]
    pub fn tracker(&self) -> &'a T {
        self.tracker
    }
    ...
    /// Allocates a new heap entry, charging the live tracker.
    ///
    /// Wrapper around [`Heap::allocate`] that supplies the borrowed tracker.
    #[inline]
    pub fn allocate(&self, data: HeapData) -> Result<HeapId, ResourceError> {
        self.heap.allocate(self.tracker, data)
    }

    /// Returns the cached `datetime.timezone.utc` singleton (lazy alloc).
    ///
    /// Wrapper around [`Heap::get_timezone_utc`] that supplies the borrowed tracker.
    #[inline]
    pub fn get_timezone_utc(&mut self) -> Result<Value, ResourceError> {
        self.heap.get_timezone_utc(self.tracker)
    }

    /// Tracks in-place memory growth of an existing heap object.
    ///
    /// Call this before performing mutations that grow containers (append,
    /// insert, extend, dict set, set add). Returns
    /// `Err(ResourceError::Memory)` if the growth would exceed configured
    /// memory limits. Does not increment the allocation count since no new
    ...
    ///
    /// Delegates to the resource tracker's `check_time()`. For
    /// `NoLimitTracker`, this is inlined as a no-op with zero runtime cost.
    /// For `LimitTracker`, it compares elapsed time against the configured
    /// `max_duration_secs`.
    ///
    ...
    ///
    /// Wrapper around [`Heap::should_gc`] that supplies the borrowed tracker.
    #[inline]
    pub fn should_gc(&self) -> bool {
        self.heap.should_gc(self.tracker)
    }

    /// Runs Bacon–Rajan trial-deletion cycle collection.
    ///
    /// Wrapper around [`Heap::collect_cycles`] that supplies the borrowed tracker.
    #[inline]
    pub fn collect_cycles(&mut self) -> usize {
        if self.heap.purple_count == 0 {
            return 0;
        }
        self.collect_cycles_inner()
    }

    /// Decrements the reference count and frees the value (plus children) once
    /// it hits zero. See [`Heap::dec_ref`] for the algorithm.
    pub fn dec_ref(&mut self, id: HeapId) {
        ...
        loop {
            ...
            let ptr = self.read_ptr(current_id);
            let heap_entry = ptr.entry(self);
            if heap_entry.refcount.get() > 1 {
                heap_entry.refcount.update(|r| r - 1);

                let is_gc_tracked = ptr.data(self).is_gc_tracked();
                if is_gc_tracked && heap_entry.color.get() != CcColor::Purple {
                    ...
                    self.heap.purple_count += 1;
                }
            } else {
                debug_assert!(
                    current_id != EMPTY_TUPLE_ID,
                    "HeapReader::dec_ref: empty-tuple singleton's heap-owned refcount must never reach zero",
                );
                assert!(
                    heap_entry.readers.get() == 0,
                    "HeapReader::dec_ref: cannot free HeapId({}) with {} active reader(s)",
                    current_id.index(),
                ...
                // on the slow path.
                let mut value = self.heap.entries.entry(current_id).expect("already looked up").free();
...
///
/// The [`ResourceTracker`] does **not** live here — it is held by persistent
/// owners (`MontyRepl`, `Snapshot`, `MontyRun`) as a sibling of `Heap`, and
/// borrowed into a [`HeapReader`] for the duration of each execution. Methods
/// that need the tracker (`allocate`, `dec_ref`, `track_growth`, …) take it as
/// an explicit `&impl ResourceTracker` parameter, or are accessed via the
/// `HeapReader` wrappers that supply it implicitly.
#[expect(
    clippy::unsafe_derive_deserialize,
    reason = "fields are leaf data; unsafe lives in HeapReader's pointer machinery, \
              not in any deserialize-driven path."
)]
#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub(crate) struct Heap {
    ...
    /// increment.
    #[serde(default, with = "serde_cell_u32")]
...
mod serde_cell_u32 {
    ...
    pub fn serialize<S: Serializer>(cell: &Cell<u32>, s: S) -> Result<S::Ok, S::Error> {
        s.serialize_u32(cell.get())
    }

    pub fn deserialize<'de, D: Deserializer<'de>>(d: D) -> Result<Cell<u32>, D::Error> {
        Ok(Cell::new(u32::deserialize(d)?))
    }
}
...
impl Heap {
    ...
    ///
    /// The resource tracker is **not** stored here — it is held by the caller
    /// and supplied to tracker-using methods via the [`HeapReader`] handle.
    ...
    /// Allocates a new heap entry, consulting the caller-supplied tracker.
    ///
    ...
    ///
    /// Most callers should use the [`HeapReader::allocate`] wrapper which
    /// supplies the tracker implicitly.
    pub fn allocate(&self, tracker: &impl ResourceTracker, data: HeapData) -> Result<HeapId, ResourceError> {
        tracker.on_allocate(|| data.py_estimate_size())?;
    ...
    pub fn get_timezone_utc(&mut self, tracker: &impl ResourceTracker) -> Result<Value, ResourceError> {
        if let Some(id) = self.timezone_utc {
            self.inc_ref(id);
            Ok(Value::Ref(id))
        } else {
            let tz = TimeZone::utc();
            let id = self.allocate(tracker, HeapData::TimeZone(tz))?;
    ...
    /// of Purple entries.
    ///
    /// Most callers should use the [`HeapReader::dec_ref`] wrapper which
    /// supplies the tracker implicitly.
    ///
    /// # Panics
    ...
    pub fn dec_ref<T: ResourceTracker>(&mut self, tracker: &T, id: HeapId) {
        HeapReader::with(self, tracker, &mut (), |reader, ()| reader.dec_ref(id));
    }
    ...
    ///
    /// Most callers should use the [`HeapReader::should_gc`] wrapper which
    /// supplies the tracker implicitly.
    #[inline]
    pub fn should_gc(&self, tracker: &impl ResourceTracker) -> bool {
        #[cfg(feature = "test-hooks")]
        if self.gc_disabled {
            return false;
        }
        ...
        let interval = tracker.gc_interval().unwrap_or(DEFAULT_GC_INTERVAL);
    ...
    /// The caller should check [`HeapReader::should_gc`] before calling
    /// this method. With `purple_count == 0` the function returns immediately
    /// without touching the heap.
    ///
    /// Most callers should use the [`HeapReader::collect_cycles`] wrapper which
    /// supplies the tracker implicitly.
    #[cfg(test)]
    pub fn collect_cycles<T: ResourceTracker>(&mut self, tracker: &T) -> usize {
        if self.purple_count == 0 {
            return 0;
        }
        ...
        HeapReader::with(self, tracker, &mut (), |reader, ()| reader.collect_cycles_inner())
    }

    fn collect_white(&mut self, tracker: &impl ResourceTracker, roots: Vec<HeapId>) -> usize {
        ...
        while let Some(id) = work_stack.pop() {
            ...
            tracker.on_free(|| value.data.0.get_mut().py_estimate_size());
            freed += 1;
...
impl<'a, T: ResourceTracker> HeapReader<'a, T> {
    ...
    fn collect_cycles_inner(&mut self) -> usize {
        ...
        let freed = self.heap.collect_white(self.tracker, roots);
...
mod tests {
    ...
    /// Shared tracker singleton so test helpers don't have to pass one around.
    const TRACKER: &NoLimitTracker = &NoLimitTracker;
    ...
    fn alloc_self_cycle(heap: &Heap) -> HeapId {
        let id = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
        let entry = heap
            .entries
            .iter()
    ...
    fn alloc_dup_child_cycle(heap: &Heap) -> (HeapId, HeapId) {
        let p_id = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
        let a_id = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
    ...
    fn cstack_only_cycle_survives_collection() {
        ...
        heap.dec_ref(TRACKER, id); // rc = 2, flagged Purple
        ...
        heap.dec_ref(TRACKER, id); // rc = 1, re-flagged Purple
        assert_eq!(heap.purple_count, 1);
        heap.collect_cycles(TRACKER);
        assert_eq!(heap.purple_count, 0);
    ...
    fn heap_read_rooted_cycle_survives_collection() {
        ...
        heap.dec_ref(TRACKER, id); // rc = 1, flagged Purple
        assert_eq!(heap.purple_count, 1);
    ...
    fn dec_ref_must_not_invalidate_live_heap_read() {
        ...
        HeapReader::with(&mut heap, TRACKER, &mut (), |heap, ()| {
    ...
    fn empty_tuple_singleton_survives_collection() {
        ...
        heap.dec_ref(TRACKER, EMPTY_TUPLE_ID);
        heap.collect_cycles(TRACKER);
        assert!(
    ...
    fn pending_purple_cycle_round_trips_through_serde() {
        ...
        heap.dec_ref(TRACKER, id); // rc 2 → 1
        assert_eq!(heap.purple_count, 1);
        ...
        restored.collect_cycles(TRACKER);
    ...
    fn isolated_cycle_with_duplicate_child_refs_is_collected() {
        ...
        heap.dec_ref(TRACKER, p_id); // P.rc 2 → 1, flagged Purple
        heap.dec_ref(TRACKER, a_id); // A.rc 4 → 3, flagged Purple
    ...
    fn cstack_rooted_cycle_with_duplicate_child_refs_collects_after_pin_dropped() {
        ...
        heap.dec_ref(TRACKER, p_id); // P.rc = 2, flagged Purple
        heap.dec_ref(TRACKER, a_id); // A.rc = 3, flagged Purple
