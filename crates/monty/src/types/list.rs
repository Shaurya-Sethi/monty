...
impl List {
    ...
    pub fn init(vm: &mut VM<'_, impl ResourceTracker>, args: ArgValues) -> RunResult<Value> {
        let value = args.get_zero_one_arg("list", &mut vm.heap)?;
        match value {
            None => {
...
impl<'h> PyTrait<'h> for HeapRead<'h, List> {
    ...
    fn py_setitem(&mut self, key: Value, value: Value, vm: &mut VM<'h, impl ResourceTracker>) -> RunResult<()> {
        ...
        mem::swap(&mut self.get_mut(&mut vm.heap).items[idx], value);
    ...
    fn py_iadd(
        ...
        if Some(*other_id) == self_id {
            ...
            self.get_mut(&mut vm.heap).items.extend(items);
        } else {
            ...
            self.get_mut(&mut vm.heap).items.extend(source_items);
...
fn call_list_method<'h>(
    ...
    match method {
        ...
        StaticStrings::Reverse => {
            ...
            list.get_mut(&mut vm.heap).items.reverse();
            Ok(Value::None)
        }
...
fn list_insert<'h>(
    ...
    let (index_obj, item) = args.get_two_args("insert", &mut vm.heap)?;
...
fn list_pop<'h>(
    ...
    let index_arg = args.get_zero_one_arg("list.pop", &mut vm.heap)?;
    ...
    Ok(list.get_mut(&mut vm.heap).items.remove(idx))
}
...
fn list_remove<'h>(
    ...
    let value = args.get_one_arg("list.remove", &mut vm.heap)?;
    defer_drop!(value, vm);
    ...
    match found_idx {
        Some(idx) => {
            ...
            let removed = list.get_mut(&mut vm.heap).items.remove(idx);
            removed.drop_with_heap(&mut vm.heap);
            Ok(Value::None)
        }
...
fn list_clear<'h>(list: &mut HeapRead<'h, List>, vm: &mut VM<'h, impl ResourceTracker>) {
    mem::take(&mut list.get_mut(&mut vm.heap).items).drop_with_heap(vm);
...
fn list_extend<'h>(
    ...
    let iterable = args.get_one_arg("list.extend", &mut vm.heap)?;
    ...
    if has_refs {
        list.get_mut(&mut vm.heap).set_contains_refs();
        vm.heap.mark_potential_cycle();
    }
    list.get_mut(&mut vm.heap).as_vec_mut().extend(items);
...
fn list_index<'h>(
    ...
    let pos_args = args.into_pos_only("list.index", &mut vm.heap)?;
...
fn list_count<'h>(
    ...
    let value = args.get_one_arg("list.count", &mut vm.heap)?;
    defer_drop!(value, vm);
...
fn do_list_sort<'h>(
    ...
    let (key_arg, reverse_arg) =
        args.extract_keyword_only_pair("list.sort", "key", "reverse", &mut vm.heap, vm.interns)?;
    ...
    apply_permutation(&mut list.get_mut(&mut vm.heap).items, &mut indices);
    Ok(())
}
...
mod tests {
    ...
    /// Test helper: runs `f` inside a fresh VM scoped to `heap` and `interns`.
    /// Wraps the [`HeapReader::with`] + [`VM::new`] dance so test bodies
    /// can stay focused on what they're actually testing.
    fn with_test_vm<R>(
        heap: &mut Heap<NoLimitTracker>,
        interns: &Interns,
        f: impl for<'h> FnOnce(VM<'h, NoLimitTracker>) -> R,
    ) -> R {
        HeapReader::with(
            heap,
            &mut (interns, PrintWriter::Disabled),
            |reader, (interns, print)| f(VM::new(reader, Vec::new(), interns, None, print.reborrow())),
        )
    }
    ...
    fn py_setitem_longint_fits_in_i64() {
        ...
        let result = with_test_vm(&mut heap, &interns, |mut vm| {
