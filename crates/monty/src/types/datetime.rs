...
use crate::{
    ...
    hash::HashValue,
    heap::{DropWithHeap, HeapData, HeapId, HeapItem, HeapRead, HeapReadOutput, HeapReader},
...
fn validate_tz_arg(value: &Value, heap: &HeapReader<'_, impl ResourceTracker>) -> RunResult<()> {
    match value {
        Value::None => Ok(()),
        Value::Ref(id) => match heap.get(*id) {
