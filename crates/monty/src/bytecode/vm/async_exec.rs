...
impl<'h, T: ResourceTracker> VM<'h, T> {
    ...
    fn await_coroutine(&mut self, mut coro: HeapRead<'h, Coroutine>) -> Result<AwaitResult, RunError> {
        ...
        coro.get_mut(&mut self.heap).state = CoroutineState::Running;
    ...
    fn await_gather_future(
        ...
        let gather_mut = gather.get_mut(&mut self.heap);
    ...
    pub(super) fn handle_task_completion(&mut self, result: Value) -> Result<AwaitResult, RunError> {
        ...
        if let Some(coro_id) = coroutine_id {
            ...
            coro.get_mut(&mut self.heap).state = CoroutineState::Completed;
        }

        // Mark task as completed and store result in task state
        let task_result = result.clone_with_heap(&self.heap);
        self.scheduler.complete_task(task_id, task_result, &mut self.heap);
        ...
        if let Some(gid) = gather_id {
            ...
            if let Some(idx) = gather_result_idx {
                gather.get_mut(&mut self.heap).results[idx] = Some(result);
            } else {
            ...
            if all_complete {
                ...
                let task_ids = mem::take(&mut gather.get_mut(&mut self.heap).task_ids);
                let results = mem::take(&mut gather.get_mut(&mut self.heap).results);
                ...
                for tid in task_ids {
                    this.scheduler.cancel_task(tid, &mut this.heap);
                }

                // Make waiter ready but don't add to ready queue since we're switching directly to it
                this.scheduler.set_state(waiter_id, TaskState::Ready, &mut this.heap);
    ...
    pub(super) fn handle_task_failure(&mut self, error: RunError) -> Result<(), RunError> {
        ...
        if let Some(gid) = gather_id {
            ...
            for tid in task_ids {
                self.scheduler.cancel_task(tid, &mut self.heap);
            }

            // Switch to waiter and propagate the error
            self.scheduler.set_state(waiter_id, TaskState::Ready, &mut self.heap);
            self.cleanup_current_task();
        ...
        self.scheduler.fail_task(task_id, error, &mut self.heap);
        self.cleanup_current_task();
        self.scheduler.set_current_task(None);
        if let Some(next_task_id) = self.scheduler.next_ready_task() {
            self.scheduler.set_current_task(Some(next_task_id));
        ...
        if let Some((gather_id, result_idx)) = this.scheduler.take_gather_waiter(call_id) {
            ...
            if pending_empty {
                ...
                if all_tasks_complete {
                    ...
                    let results: Vec<Value> = mem::take(&mut gather.get_mut(&mut self.heap).results)
                        .into_iter()
                        .map(|r| r.expect("all results should be filled when gather is complete"))
                        .collect();
                    ...
                    if waiter_context_in_vm {
                        ...
                        self.scheduler.make_ready(waiter_id, &mut self.heap);
                    }
                }
            }
        } else {
    ...
    pub fn fail_future(&mut self, call_id: u32, error: RunError) {
        ...
        self.scheduler.fail_for_call(call_id, error, &mut self.heap);
    }
