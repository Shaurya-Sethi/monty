...
///
/// # Lifetime
/// * `'h` — Single, **invariant** lifetime gating all of the VM's borrows: the heap
///   (via `HeapReader`), interns, module code, call frames, and print writer
///   references. Invariance is required for the heap-reader brand; the other borrows
///   are covariant in their lifetime so they can be subtyped down to `'h` at
///   construction time. The HRTB on `VM::with` / `VM::with_restore` makes `'h`
///   universally quantified per call, preventing `HeapRead<'h, _>` handles from
...
impl<'h, T: ResourceTracker> VM<'h, T> {
    /// Constructs a fresh VM that owns the given heap reader.
    ///
    /// This is *not* itself a safety boundary — it just packages the components
    /// into a `VM<'h, T>`. The brand-uniqueness invariant on `'h` is enforced
    /// upstream by [`HeapReader::with`], which is the sole way to obtain a
    /// `HeapReader<'h, T>` with an HRTB-quantified `'h`. Pass that reader (and
    /// the env-borrowed bits that the with-call's `data` argument rebrands to
    /// `'h`) straight into this constructor.
    pub fn new(
        heap: HeapReader<'h, T>,
        globals: Vec<Value>,
        interns: &'h Interns,
    ...
    ///
    /// Same construction shape as [`Self::new`]; the brand-uniqueness gating
    /// lives in [`HeapReader::with`]. `FunctionId`s in `snapshot.frames`
    /// are resolved against `interns`; frames with no function ID resolve to
    /// `module_code`.
    pub fn restore(
        heap: HeapReader<'h, T>,
        snapshot: VMSnapshot,
    ...
    ///
    /// Uses the `module_code` provided at [`VM::with`] / [`VM::with_restore`] time.
    /// Panics if the VM was not initialized with module code.
    pub fn run_module(&mut self) -> Result<FrameExit, RunError> {
        let code = self.module_code.expect("module_code must be set on the VM");
...
impl<'h, T: ResourceTracker> ContainsHeapReader<'h> for VM<'h, T> {
    type ResourceTracker = T;
