...
///
/// `a.get(reader)` borrows `reader` immutably, and the resulting slice keeps that borrow alive.
/// `reader.heap.heap_mut().allocate(...)` requires mutable access to `reader.heap`, which
/// conflicts with the live immutable borrow.
///
/// Expected: E0502 (cannot borrow `*reader.heap` as mutable because it is also borrowed as immutable)
...
fn heap_mutation_while_reading(list_id: HeapId, heap: &mut Heap<impl ResourceTracker>) {
    HeapReader::with(heap, |reader| {
        ...
        let _ = reader.heap.heap_mut().allocate(HeapData::Str(Str::new("boom".into())));
        let _ = slice.len();
    });
}
...
///
/// Expected: E0499 (cannot borrow `*reader` as mutable more than once at a time)
...
fn dec_ref_while_reading(list_id: HeapId, heap: &mut Heap<impl ResourceTracker>) {
    HeapReader::with(heap, |reader| {
        ...
        let list_ref = a.get(reader);
        reader.heap.heap_mut().dec_ref(list_id);
...
///
/// The iterator borrows the slice (which keeps `reader` immutably borrowed), and the
/// closure attempts to capture `reader.heap` for mutable access.
///
/// Expected: E0500 (closure requires unique access to `*reader.heap` but it is already borrowed)
...
fn mutation_in_map_closure(list_id: HeapId, other_id: HeapId, heap: &mut Heap<impl ResourceTracker>) {
    HeapReader::with(heap, |reader| {
        ...
        let result: Vec<bool> = a
            ...
            .iter()
            .map(|_v| {
                reader.heap.heap_mut().dec_ref(other_id);
                true
            })
            .collect();
        let _ = result;
...
///
/// Expected: E0502 (cannot borrow `*reader` as mutable because it is also borrowed as immutable)
...
fn read_while_ref_alive(id_a: HeapId, id_b: HeapId, heap: &mut Heap<impl ResourceTracker>) {
    HeapReader::with(heap, |reader| {
        let a = match reader.read(id_a) {
            HeapReadOutput::List(list) => list,
            _ => unreachable!(),
        };
        let a_ref = a.get(reader);
        let _b = reader.read(id_b);
